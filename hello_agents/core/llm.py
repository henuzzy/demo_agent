...
from hello_agents.core import HelloAgentsException

# 支持的供应商
# Literal作用：限制值只能是某几个固定的字面量。
...
class HelloAgentsLLM:
        ...
        def _auto_detect_provider(self, api_key: str | None, base_url: str | None) -> str:
            ...
            if actual_api_key:
                ...
                if actual_key_lower.startswith("ms-"):
                    ...
                    pass
                elif actual_key_lower.endswith(".") or "." in actual_api_key[-20:]:
                    # 智谱AI的API密钥格式通常包含点号
                    return "zhipu"
            ...
            if actual_base_url:
                actual_url_lower = actual_base_url.lower()
                if "api.openai.com" in actual_url_lower:
                    return "openai"
                elif "api.deepseek.com" in actual_url_lower:
                    return "deepseek"
                elif "dashscope.aliyuncs.com" in actual_url_lower:
                    return "qwen"
                elif "api-inference.modelscope.cn" in actual_url_lower:
                    return "modelscope"
                elif "api.moonshot.cn" in actual_url_lower:
                    return "kimi"
                elif "open.bigmodel.cn" in actual_url_lower:
                    return "zhipu"
                elif "localhost" in actual_url_lower or "127.0.0.1" in actual_url_lower:
                    if ":11434" in actual_url_lower or "ollama" in actual_url_lower:
                        return "ollama"
                    elif ":8000" in actual_url_lower and "vllm" in actual_url_lower:
                        return "vllm"
                    elif ":8080" in actual_url_lower or ":7860" in actual_url_lower:
                        return "local"
                    else:
                        # 根据API密钥进一步判断
                        if actual_api_key and actual_api_key.lower() == "ollama":
                            ...
                            return "local"
                elif any(port in actual_url_lower for port in [":8080", "::7860", ":5000"]):
                    return "local"
            # 4. 默认返回auto，使用通用配置
            return "auto"
    ...
    def invoke(self, messages: list[dict[str,str]], **kwargs) ->str:
        ...
        try:
            ...
            raise HelloAgentsLLM(f"LLM调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: list[dict[str,str]], **kwargs):
        ...
        # yield → 单次产出一个值。
        # yield from → 委托另一个生成器 / 可迭代对象，自动逐个产出它的值。
