...
# 获取一个和当前模块绑定的日志记录器对象。
# __name__:是 Python 内置变量，表示当前模块的名字
logger = logging.getLogger(__name__)


# load_dotenv() 会读取当前目录下的 .env 文件，把里面的键值对加载到系统环境变量里。
# 加载后，你就可以用 os.getenv("API_KEY") 或 os.environ["API_KEY"] 来获取这些值。
load_dotenv()
...
class QdrantConfig(BaseModel):
    ...
    # 连接配置
    url: str | None = Field(description="Qdrant服务URL (云服务或自定义URL)")
    api_ley: Optional[str] = Field(default=None, description="Qdrant API密钥 (云服务需要)")
    ...
    def from_env(cls) -> "QdrantConfig":
        ...
        return cls(
            ...
            api_ley = os.getenv("QDRANT_API_KEY"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典"""
        # exclude_none = True:表示在导出字典时，忽略掉值为None的字段。
        # 这是Pydanticv2的方法（在v1里叫dict()）。会把模型实例的所有字段和值导出成一个dict。
...
class DatabaseConfig(BaseModel):
    ...
    def validate_connections(self) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.info(f"Neo4j连接成功：{'成功' if results["neo4j"] else '失败'}")
        except Exception as e:
            results["neo4j"] = False
