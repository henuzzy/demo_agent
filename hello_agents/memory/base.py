...
class MemoryItem(BaseModel):
    ...
    # class Config 是 Pydantic 模型的内部配置类，用来控制模型行为。它必须写在模型内部，不能写在外面。
    # Pydantic是一个用来“定义数据结构 + 自动校验数据”的Python库。
    # 你写一个类，Pydantic 自动帮你检查数据类型、转换数据格式、保证数据合法。
    class Config:
...
class BaseMemory(ABC):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # self.__class__ 是当前对象所属的类，__name__ 是类的名字
