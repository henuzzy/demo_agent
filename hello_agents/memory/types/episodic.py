...
from hello_agents.memory.base import BaseMemory, MemoryItem, MemoryConfig
from hello_agents.memory.storage import SQLiteDocumentStore, QdrantVectorStore
...
class EpisodicMemory(BaseMemory):
    ...
    def find_patterns(self, user_id: str = None, min_frequency: int = 2) -> List[Dict[str, Any]]:
        """发现用户行为模式"""
        # 检查缓存：如果已经分析过1小时内的模式，直接返回缓存，避免重复计算
